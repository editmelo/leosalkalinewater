...
describe("computeTotals", () => {
  ...
  it("weekly: fixed $55/mo regardless of jugCount", () => {
    const t = computeTotals({ planId: "weekly", jugCount: 5, zip: "46204", customerType: "residential" });
  ...
  it("biweekly: fixed $30/mo", () => {
    const t = computeTotals({ planId: "biweekly", jugCount: 1, zip: "46204", customerType: "residential" });
    expect(t.subtotalCents).toBe(3000);
  });

  it("starter: fixed $45 one-time", () => {
    const t = computeTotals({ planId: "starter", jugCount: 1, zip: "46204", customerType: "residential" });
