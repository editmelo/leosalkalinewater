...
describe("buildOrderPayload", () => {
  it("biweekly: recurring=true, cadence MONTHLY, deliveryFrequency biweekly", () => {
    const p = buildOrderPayload({ planId: "biweekly", jugCount: 1, zip: "46204", customerType: "residential" });
  ...
  it("payg: recurring=false, cadence null, subtotal $20 for 1 jug", () => {
    const p = buildOrderPayload({ planId: "payg", jugCount: 1, zip: "46204", customerType: "residential" });
    expect(p.recurring).toBe(false);
    expect(p.cadence).toBeNull();
    expect(p.totals.subtotalCents).toBe(2000);
  });

  it("customerType is passed through to top-level payload field", () => {
    const p = buildOrderPayload({ planId: "payg", jugCount: 2, zip: "46204", customerType: "business" });
